...
/**
 * TopoField extracted from MengTo/threeui (MIT) — src/shaders/neuform-isolated/NeuformBatchEffects.tsx.
 * Adapted to render on a blue paper palette: light-blue background with royal-blue contour lines.
 */
...
<body class="bg-black text-white min-h-screen relative overflow-x-hidden" style="background-color: #000; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale;">
    ...
    <!-- Main Content (hidden by the isolation pipeline; kept for structural parity) -->
...
function patchTopoField(
  ...
  if (mode === "light") {
    // Soft blue-paper backdrop: a barely-visible trend of contour lines, no strong grid.
    next = next
      .replace(
        "gridLines = clamp(gridLines, 0.0, 1.0) * 0.12;",
      ...
      .replace(
        "float topoLines = smoothstep(0.02, 0.00, triangleWave) * 0.45;",
        "float topoLines = smoothstep(0.03, 0.00, triangleWave) * 0.35;",
      )
      .replace(
        `vec3 color = vec3(0.0);
                    color += vec3(1.0) * gridLines;
                    color += vec3(1.0) * topoLines;`,
        `vec3 paper = vec3(0.86, 0.89, 0.98);
                    vec3 ink = vec3(0.4, 0.5, 0.78);
 ...
 * time. Ships as a sandboxed iframe so the shader's own script/canvas loop stays self-contained.
 */
